...
class ZoneChoice(click.Choice):
    def __init__(self):
        choices = []
        choices.extend(
            tpunicorn.tpu.get_zone_abbreviations(only_unambiguous_results=True)
        )
...
def tpu_zone_option():
    return click.option(
        ...
        + ", ".join(tpunicorn.tpu.get_zone_abbreviations(only_unambiguous_results=True))
        + "}",
    )
...
@click.option(
    "-t", "--tpu", type=click.STRING, help="List a specific TPU by id.", multiple=True
)
@click.option(
    "-s",
    "--silent",
    is_flag=True,
    help="If listing a specific TPU by ID, and there is no such TPU, don't throw an error.",
)
...
def wait_healthy(tpu, zone=None, project=None, color=True):
    ...
    while True:
        if check_healthy(tpu, zone=zone, project=project, color=color):
            return
        click.echo(
            "TPU {} not yet healthy; waiting 30 seconds...".format(
...
def do_step(
    label=None, command=None, dry_run=False, delay_after=1.0, args=(), kwargs={}
):
...
@click.argument(
    "tpu",
    type=click.STRING,
    metavar='[TPU; default="0+"]',
    default="0+",
    shell_complete=complete_tpu_id,
)
@tpu_zone_option()
@click.option(
    "-v",
...
@click.option(
    "-a",
    "--accelerator-type",
    metavar='[ACCELERATOR_TYPE; default="v2-8"]',
    type=click.STRING,
    default=None,
)
@click.option(
    "-dd", "--data-disk", metavar="[DATA_DISK]", type=click.STRING, default=None
)
@click.option("--async", "async_", is_flag=True)
@click.option(
    "-d", "--description", metavar="DESCRIPTION", type=click.STRING, default=None
)
@click.option(
    "-n",
    "--network",
    metavar='[NETWORK; default="default"]',
    type=click.STRING,
    default="default",
)
@click.option(
...
def create(
    ...
    ):  # --range appears to be broken on TPU VMs for now; don't give a default
        if cores == 8:
    ...
    if range is not None and range.startswith("10.48.") and cores > 8:
        raise ValueError(
            "The range {range!r} conflicts with the default 10.48.* range of v2-8's and v3-8's. I decided to raise an error rather than a warning, because we rely on this specific range for our own internal networking. If you're making a TPU pod, try a different index other than {index}. If you really, really wanted to use 10.48.* for you TPU pods, I'm very sorry; ping me on twitter (@theshawwn) and I'll change this.".format(
    ...
    try:
        ...
        tpu = "tpu-{accelerator_type}-{zone_abbrev}-{index}".format(
            accelerator_type=accelerator_type, zone_abbrev=zone_abbrev, index=index
        )
    except ValueError:
        pass
    if project is None:
        project = tpunicorn.tpu.get_default_project()
    create = tpunicorn.create_tpu_command(
        tpu,
        zone=zone,
        version=version,
...
def delete(tpu, zone, project, yes, dry_run, async_):
    ...
    click.echo("")
    delete = tpunicorn.delete_tpu_command(
        tpu, zone=zone, project=project, async_=async_
    )
    create = tpunicorn.create_tpu_command(
        tpu, zone=zone, project=project, async_=async_
    )
    ...
    print_step(
        "You {} recreate the TPU with:".format("could then" if dry_run else "can"),
        create,
    )
...
def stop(tpu, zone, project, yes, dry_run, async_):
    ...
    print_step(
        "You {} restart the TPU with:".format("could then" if dry_run else "can"), start
    )
...
def start(tpu, zone, project, yes, dry_run, async_):
    ...
    print_step(
        "You {} stop the TPU with:".format("could then" if dry_run else "can"), stop
    )
...
def reimage(tpu, zone, project, version, yes, dry_run, async_):
    ...
    tpu = tpunicorn.get_tpu(tpu=tpu, zone=zone, project=project)
    reimage = tpunicorn.reimage_tpu_command(
        tpu, zone=zone, project=project, version=version, async_=async_
    )
...
@click.option(
    "--retry",
    type=int,
    help="if the TPU creation fails (due to capacity errors or otherwise), "
...
@click.option(
    ...
    type=float,
    default=1.0,
    help="multiply retry time by a float between 1 and retry_randomness",
)
@click.option(
    "--accelerator_type",
    type=click.STRING,
...
def recreate(
    ...
    **kws,
    ...
    try:
        tpu = tpunicorn.get_tpu(tpu=tpu, zone=zone, project=project)
        click.echo(
            "Current status of TPU {} as of {}:".format(
    ...
    while do_step("Step 2: create TPU...", create, dry_run=dry_run) != 0:
        if retry is None:
            click.echo(
                "TPU {} failed to create (is the region out of capacity?)".format(
                    tpunicorn.tpu.parse_tpu_id(tpu)
                ),
                err=True,
            )
        ...
        click.echo(
            "TPU {} failed to create; trying again in {} minutes...".format(
                tpunicorn.tpu.parse_tpu_id(tpu), int((retry * n) // 60)
            ),
            err=True,
        )
        time.sleep(retry * n)
...
def ssh(tpu, zone, project, yes, dry_run, ssh_flag):
    ...
    print_tpu_status(tpu)
    click.echo("")
    cmd = tpunicorn.tpu.ssh_tpu_command(
        tpu, zone=zone, project=project, ssh_flag=ssh_flag
    )
    if not yes:
...
def install_completion(shell, yes, dry_run):
    def install_completion(path, script, name):
        ...
        if dry_run:
            click.secho(
                "Dry run; not writing. Would have appended to {} the following text:".format(
                    path
                ),
                bold=True,
            )
            ...
            with click.open_file(path, "w", atomic=True) as f:
                f.write(contents)
            click.secho(
                "{} completion installed for `{}`".format(shell, name), bold=True
            )

    scripts = [
        completions[shell]["script"].format(binary.upper().replace("-", ""), binary)
    ...
    for script, name in zip(scripts, binary_names):
        ...
        tasks.append(
            [
                "Step {}: Append the completion script for {} to {}".format(
                    len(tasks) + 1, name, path
                ),
                install_completion,
                (path, script, name),
                {},
