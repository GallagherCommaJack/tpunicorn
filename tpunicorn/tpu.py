...
def build_commandline(cmd, tpu_name, *args, **kws):
    ...
    return " ".join(
        ...
        + [build_opt(k, v) for k, v in kws.items() if v is not None]
    )
...
def parse_tpu_zone(tpu):
    ...
    if isinstance(tpu, str):
        ...
        results = [
            [expand_zone_abbreviations(k), re.findall(r"\b{}\b".format(k), fqn)]
...
def get_zone_abbreviations(
    full_zone_names=None, only_unambiguous_results=False
):  # e.g. ['europe-west4-a']
        ...
        for cshort, cfull in country_abbrevs.items():
            for rshort, rfull in region_abbrevs.items():
                if cfull == country and rfull == region:
                    # e.g. 'euw4a'
                    results[cshort + rshort + region_id + zone_id].append(
                        full_zone_name
                    )
...
def cache_clearer(x):
    if callable(x):
        if (
            hasattr(x, "ring")
            and hasattr(x, "delete")
            and x.ring.__module__.split(".")[0] == "ring"
        ):
            return lambda: x.delete()
        if (
            hasattr(x, "cache_clear")
            and x.__class__.__module__.split(".")[0] == "functools"
        ):
...
def reset_caches(module=__name__, matching="", exclude=""):
    ...
    for k, v in sys.modules[module].__dict__.items():
        ...
        if uncache(v):
            logger.info(
                "Cache was reset: {module}.{name}".format(module=module, name=k)
            )
...
def request_urls(path, api="tpu", apiVersion="v2alpha1", **kws):
    if "/" not in path:
        *parts, resource = path.split(".")
        ...
        path = "/".join(
            [
                "/".join([part, "{" + replacements.get(part[:-1], part[:-1]) + "}"])
                for part in parts
            ]
            + [resource]
        )
    ...
    return list(
        braceexpand.braceexpand(
            url.format(api=api, apiVersion=apiVersion, **bracify(**kws))
        )
    )
...
def request(
    path, api="tpu", apiVersion=None, headers=None, session=None, project=None, **kws
):
...
def api_list_locations_url(project=None):
    project = get_default_project(project=project)
    return "https://tpu.googleapis.com/v2alpha1/projects/{project}/locations?alt=json".format(
        project=project
    )
...
def api_list_nodes_url(zone, project=None):
    project = get_default_project(project=project)
    return "https://tpu.googleapis.com/v2alpha1/projects/{project}/locations/{zone}/nodes?alt=json".format(
...
def api_zones(project=None, session=None):
    return [
        x["locationId"] for x in api_list_locations(project=project, session=session)
    ]
...
def list_tpus(zone=None, project=None, session=None):
    if zone is None:
        ...
        tpus = request(
            "projects.locations.nodes", zone=zone, project=project, session=session
        ).get("nodes", [])
...
def parse_tpu_master(tpu, internal_only=True):
    return "{}:{}".format(
        parse_tpu_ip(tpu, internal_only=internal_only), parse_tpu_port(tpu)
    )
...
def parse_tpu_data_disk(tpu, disk_index=0):
    ...
    if disk_index >= 0 and disk_index < len(disks):
        ...
        return "source={source},mode={mode}".format(
            source=disk["sourceDisk"], mode=disk["mode"].lower().replace("_", "-")
        )
...
def create_tpu_command(
    ...
    if data_disk is not None:
        if project is None or zone is None:
            raise ValueError(
                "When --data-disk is specified, you must also specify --zone and --project"
            )
